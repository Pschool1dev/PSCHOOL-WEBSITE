...
const MesEnfants = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ prenom: '', nom: '', age: '', niveau_etude: '', nom_ecole: '', localite_ecole: '', formations_interet: [] });
      fetchChildren();
    } catch (error) {
  ...
  if (loading) return <div className="flex justify-center p-20"><div className="animate-spin h-8 w-8 border-b-2 border-emerald-500 rounded-full"></div></div>;

  return (
    <div className="p-6 bg-[#FBFDFB] min-h-screen">
      ...
      {/* Badge Succès */}
      {successData && (
        <div className="mb-10 p-6 bg-white rounded-3xl border border-emerald-100 shadow-sm flex flex-col md:flex-row items-center gap-6 animate-in fade-in slide-in-from-top-4">
            <div className="bg-emerald-50 p-2 rounded-xl">
                <QRCodeSVG value={`${window.location.origin}/login/auto?token=${successData.credentials?.access_token || ''}`} size={80} />
            </div>
            <div className="flex-1">
                ...
                <div className="flex gap-4 mt-2">
                    <div className="text-xs"><span className="text-slate-400">ID :</span> <span className="font-medium text-slate-700">{successData.credentials?.username}</span></div>
                    <div className="text-xs"><span className="text-slate-400">PASS :</span> <span className="font-medium text-slate-700">pschool2026</span></div>
                </div>
            </div>
            <button onClick={() => window.print()} className="text-xs font-semibold text-emerald-600 hover:bg-emerald-50 px-4 py-2 rounded-lg transition-colors border border-emerald-100">Imprimer le badge</button>
        </div>
      )}
              ...
              <tr key={child.id} className="hover:bg-slate-50/50 transition-colors">
                <td className="px-6 py-4 flex items-center gap-4">
                  ...
                  <div>
                      ...
                      <p className="text-[10px] text-slate-400 font-medium">{child.nom_ecole || 'Établissement'} • {child.niveau_etude}</p>
                  </div>
                </td>
                ...
                <td className="px-6 py-4">
                  <div className="flex justify-end gap-2">
                    ...
                    <button onClick={() => handleDelete(child.id)} className="p-2 text-slate-300 hover:text-red-500 transition-colors">
      ...
      {/* Modal avec Localité École ajoutée */}
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-in zoom-in duration-300">
            <div className="px-8 py-5 border-b border-slate-50 flex justify-between items-center">
                ...
                <button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-red-500"><HiX className="h-5 w-5" /></button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                      ...
                      <input required type="text" className="w-full p-3 bg-slate-50 border border-slate-100 focus:border-emerald-500 focus:bg-white rounded-xl outline-none text-sm transition-all" value={formData.prenom} onChange={(e) => setFormData({...formData, prenom: e.target.value})} />
                  </div>
                  <div className="space-y-1">
                      <label className="text-[10px] font-semibold text-slate-400 uppercase ml-1">Nom</label>
                      <input required type="text" className="w-full p-3 bg-slate-50 border border-slate-100 focus:border-emerald-500 focus:bg-white rounded-xl outline-none text-sm transition-all" value={formData.nom} onChange={(e) => setFormData({...formData, nom: e.target.value})} />
                  </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                      ...
                      <input required type="number" className="w-full p-3 bg-slate-50 border border-slate-100 focus:border-emerald-500 focus:bg-white rounded-xl outline-none text-sm transition-all" value={formData.age} onChange={(e) => setFormData({...formData, age: e.target.value})} />
                  </div>
                  <div className="space-y-1">
                      <label className="text-[10px] font-semibold text-slate-400 uppercase ml-1">Niveau d'étude</label>
                      <select required className="w-full p-3 bg-slate-50 border border-slate-100 focus:border-emerald-500 rounded-xl outline-none text-sm" value={formData.niveau_etude} onChange={(e) => setFormData({...formData, niveau_etude: e.target.value})}>
              ...
              <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                      ...
                      <input required type="text" placeholder="Nom de l'école" className="w-full p-3 bg-slate-50 border border-slate-100 focus:border-emerald-500 rounded-xl outline-none text-sm" value={formData.nom_ecole} onChange={(e) => setFormData({...formData, nom_ecole: e.target.value})} />
                  </div>
                  <div className="space-y-1">
                      <label className="text-[10px] font-semibold text-slate-400 uppercase ml-1">Ville / Quartier</label>
                      <input required type="text" placeholder="Localité" className="w-full p-3 bg-slate-50 border border-slate-100 focus:border-emerald-500 rounded-xl outline-none text-sm" value={formData.localite_ecole} onChange={(e) => setFormData({...formData, localite_ecole: e.target.value})} />
                  </div>
              </div>
              ...
              <div className="p-4 bg-emerald-50/30 rounded-xl border border-emerald-50 space-y-3">
                ...
                <div className="grid grid-cols-1 gap-2 max-h-28 overflow-y-auto pr-1 custom-scrollbar">
                  {availableFormations.map((f) => (
                    <label key={f.id} className="flex items-center gap-3 p-2.5 bg-white rounded-lg border border-slate-100 cursor-pointer hover:border-emerald-200 transition-all text-xs">
                        <input type="checkbox" className="w-4 h-4 rounded text-emerald-600 focus:ring-0" checked={formData.formations_interet.includes(f.titre)} onChange={() => handleCheckboxChange(f.titre)} />
              ...
              <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-emerald-600 text-white rounded-xl font-semibold text-sm hover:bg-emerald-700 transition-all disabled:opacity-50 mt-2">
